...
def process_file_chunks(csv_path, target_signal, target_cycles, skip_rows):
    ...
    return {"Stretch": stretch, "Twist": twist, "Freq": nominal_freq, "raw_ratios": np.array(ratios)}
...
def plot_matrix(matrix, pval_matrix, metric_name, show, save_path, signal_type, decimals=4):
    plot_df = matrix.copy()
    ...
    fig.update_layout(
        title=dict(
            text=f"Amplitude {metric_name} Matrix for {signal_type.capitalize()} Signal",
            font=dict(size=30)
        ),
...
def main():
    parser = argparse.ArgumentParser(description="Process DAQ CSV files into an Amplitude Matrix.")
    ...
    if matrix is not None:
        print(f"\n--- {metric_name} Matrix ---")
        print(matrix.round(args.decimals))
        
        plot_matrix(matrix, pval_matrix if args.stats else None, metric_name, args.plot, args.save_plot, args.signal, args.decimals)
